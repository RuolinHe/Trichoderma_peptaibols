...
%     reference_motif_path='D:/Shared with me/MATLAB_toolbox/reference_motif';
    reference_motif_path='/storage/disk1/HRL/Project/SM_in_genome/analysis/tools_1025/reference_motif';
end
if nargin<2||isempty(Pfam_database_path)
%     Pfam_database_path='D:/cygwin64/home/74005/Pfam';
    Pfam_database_path='/storage/disk1/HRL/Tool/Pfam';
end
result=[];
for i = 1:length(seq)
...
for ii = 1:length(seq) % for each sequence
    ...
    %         if ((str2double(hmm_r(i,21))-str2double(hmm_r(i,20))+1)>length_threshold*str2double(hmm_r(i,3)))||(strcmp(hmm_r{i,1},'Thioesterase')&&(str2double(hmm_r(i,21))-str2double(hmm_r(i,20))+1)>0.5*str2double(hmm_r(i,3))) % the length of matching shouldn't be too short (>length_threshold*domain_length)
        ...
        if loop_S_code_judge
            ...
            loc_result.loop_group=Loop_length2group_HRL(reference_motif_path,loop_length);
        end
    else
        loc_result.domain_list=[];
        loc_result.motifid_mat=[];
        loc_result.seq_list=[];
        loc_result.index_mat=[];
        loc_result.C_subtype_list=[];
        loc_result.C_score=[];
        loc_result.Raw_C_score_list=[];
        loc_result.C_subtype_str=[];
        if loop_S_code_judge
            loc_result.loop_length=[];
            loc_result.loop_seq=[];
            loc_result.S_code=[];
            loc_result.loop_group=[];
        end
    end
